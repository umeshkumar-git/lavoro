...
test('health endpoint is available and reports healthy status', async () => {
  ...
  assert.equal(payload.service, 'Developer Mentor AI', 'service name should match the app');
  assert.equal(payload.frontend, 'vanilla-html-css-js', 'frontend should be the static app shell');
});
...
test('homepage serves the app shell with the premium dashboard HTML', async () => {
  ...
  assert.match(html, /Developer Mentor AI/, 'page should include the application title');
  assert.match(html, /app-shell/, 'page should render the app shell container');
